...
export const TraktScreen = Object.assign(Object.create(SettingsScreen), {
  ...
  renderTrackingSection() {
    ...
    const connectedLibrarySources = [
      { id: TraktLibrarySourceMode.LOCAL, label: "Nuvio Enhanced" },
      ...
      <section class="settings-slide-panel settings-trakt-panel settings-tracking-panel">
        ...
        <div class="settings-trakt-scroll-area">
          ...
          <div class="settings-trakt-card">
            ...
            <p class="settings-tracking-card-subtitle">${escapeHtml(t("tracking_sources_subtitle", {}, "Choose where Nuvio Enhanced reads your library and watch progress. Playback scrobbles to every connected service."))}</p>
            <div class="settings-trakt-options-stack">
              ${this.renderActionRow({ focusKey: "tracking:librarySource", title: t("trakt_library_source_title", {}, "Library source"), subtitle: t("tracking_library_source_dialog_subtitle", {}, "Choose the service Nuvio Enhanced reads for your Library."), value: connectedLibrarySources.find((item) => item.id === settings.librarySourceMode)?.label || "Nuvio Enhanced" })}
              ${this.renderActionRow({ focusKey: "tracking:progressSource", title: t("trakt_watch_progress_source_title", {}, "Watch progress source"), subtitle: t("tracking_watch_progress_dialog_subtitle", {}, "Choose the service Nuvio Enhanced reads for resume and Continue Watching."), value: connectedWatchSources.find((item) => item.id === settings.watchProgressSource)?.label || "Nuvio Sync" })}
            </div>
          </div>
          ${
            traktConnected
